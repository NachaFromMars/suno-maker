...
# ====== Claude/Anthropic Provider Config ======
# Reads ANTHROPIC_API_KEY (or CLAUDE_API_KEY) from env or ~/.suno/.env
_ANTHROPIC_KEY = os.environ.get("ANTHROPIC_API_KEY", "") or os.environ.get("CLAUDE_API_KEY", "")
...
if not _ANTHROPIC_KEY:
    _env_file = os.path.expanduser("~/.suno/.env")
    if os.path.exists(_env_file):
        with open(_env_file) as _f:
            for _line in _f:
                ...
                elif _line.startswith("CLAUDE_API_KEY="):
                    _ANTHROPIC_KEY = _line.split("=", 1)[1].strip().strip('"').strip("'")
                elif _line.startswith("ANTHROPIC_MODEL="):
                    _ANTHROPIC_MODEL = _line.split("=", 1)[1].strip().strip('"').strip("'")
                elif _line.startswith("ANTHROPIC_BASE_URL="):
                    _ANTHROPIC_BASE_URL = _line.split("=", 1)[1].strip().strip('"').strip("'")
# ====== End Claude config ======

# ====== 确保 hcaptcha-challenger 支持 Suno 自定义 hCaptcha 域名 ======
# Suno 使用 hcaptcha-assets-prod.suno.com 而非标准 newassets.hcaptcha.com
# ALSO: Inject AnthropicProvider into all 4 reasoners (replacing Gemini)
try:
    ...
    def _patched_init(self, *args, **kwargs):
        ...
        # ====== Inject Claude provider into all 4 reasoners ======
        if _ANTHROPIC_KEY:
            ...
            self._challenge_router._provider = claude_provider
            self._image_classifier._provider = claude_provider
            self._spatial_path_reasoner._provider = claude_provider
            self._spatial_point_reasoner._provider = claude_provider
            print(f"   ✅ Claude provider injected (model={_ANTHROPIC_MODEL})", flush=True)
        else:
            print("   ⚠️ No ANTHROPIC_API_KEY found, will use Gemini fallback", flush=True)

    RoboticArm.__init__ = _patched_init

    print("   ✅ hCaptcha domain compat + Claude provider patch applied", flush=True)
except Exception as e:
    print(f"   ⚠️ hCaptcha patch skipped: {e}", flush=True)
# ====== Patch 结束 ======
...
def main():
    ...
    parser.add_argument("--anthropic-key", type=str, default="",
    ...
    parser.add_argument("--gemini-key", type=str, default=os.environ.get("GEMINI_API_KEY", ""),
                        help="Gemini API Key (fallback if no Anthropic key)")
    ...
    # Check API keys — Claude first, Gemini fallback
    # Claude key is loaded globally (top of file) into _ANTHROPIC_KEY
    # Override from CLI if provided
    global _ANTHROPIC_KEY
    if args.anthropic_key:
        _ANTHROPIC_KEY = args.anthropic_key
    ...
    if not _ANTHROPIC_KEY and not gemini_key:
        print("❌ No API key set! hCaptcha cannot be solved automatically.", flush=True)
        print("   Option 1 (Claude): export ANTHROPIC_API_KEY='sk-ant-...'", flush=True)
        print("   Option 2 (Gemini): export GEMINI_API_KEY='your_key'", flush=True)
        print("   Or add to ~/.suno/.env", flush=True)
        sys.exit(1)

    # If no gemini key but have claude, set a dummy gemini key
    # (AgentConfig requires it, but our monkey-patch overrides the actual provider)
    if not gemini_key and _ANTHROPIC_KEY:
        gemini_key = "claude-override-dummy-key"

    # 显示环境信息
    is_headless = _is_headless_linux()
    captcha_engine = f"Claude ({_ANTHROPIC_MODEL})" if _ANTHROPIC_KEY else f"Gemini"
    print("=" * 60, flush=True)
    print("🎵 Suno Song Creator (Headless Linux + Claude captcha)", flush=True)
    print(f"   Title: {args.title}", flush=True)
    print(f"   Style: {args.style}", flush=True)
    print(f"   Lyrics: {lyrics[:60]}{'...' if len(lyrics)>60 else ''}", flush=True)
    print(f"   Captcha Engine: {captcha_engine}", flush=True)
    print(f"   Environment: {'Linux headless (Xvfb)' if is_headless else platform.system() + ' (GUI)'}", flush=True)
    print("=" * 60, flush=True)
